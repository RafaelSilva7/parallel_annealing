...
bool write_data(OtsuArgs *arg1, OtsuArgs *arg2, pair<int,float> best, int n_it, float temp, double time_r);
...
int main(int argc, char const *argv[])
{
    ...
    // create struct data of thread1
    OtsuArgs otsu_arg2;
    otsu_arg2.population.clear();
    otsu_arg2.x_min = 128;
    otsu_arg2.x_max = 255;
    otsu_arg2.n_it = 0;

    // initialize thread1
    err1 = pthread_create(&ntid1, NULL, simu_otsu, (void*)&otsu_arg1);
    if (err1 != 0)
        printf("can't create thread1"); // thread 1
        
    // initialize thread2
    err2 = pthread_create(&ntid2, NULL, simu_otsu, (void*)&otsu_arg2);
    if (err2 != 0)
        printf("can't create thread2"); // thread 1

    // finalize threads
    void *tret1, *tret2;
    pthread_join(ntid1, &tret1);
    pthread_join(ntid2, &tret2);
    ...
    pair<int, float> thresh_thread2 = otsu_arg2.population[otsu_arg2.population.size()-1];

    pair<int, float> best;
    int n_it;
    float temp;
    if (thresh_thread1.second > thresh_thread2.second){
        best = thresh_thread1;
        n_it = otsu_arg1.n_it;
        temp = otsu_arg1.temp;
    }else {
    ...
    // show best solution
    printf("\nTime running: %.4fs\n", runtime);
    printf("[thread1] threshold : %d -> %f\n", thresh_thread1.first, thresh_thread1.second);
    printf("[thread2] threshold : %d -> %f\n", thresh_thread2.first, thresh_thread2.second);
    printf("best threshold: %d\n", best.first);

    // create new image for segmentation
    cv::Mat * img_output = segmentation(&img_input, best.first);
    ...
    write_data(&otsu_arg1, &otsu_arg2, best, n_it, temp, runtime);
    printf("writed data...\n");
...
{
    fstream myfile;
    myfile.open("pararel_annealing/data/population1.csv", std::fstream::out);
    myfile << "threshold;energy" << endl;
    for (auto it: arg1->population)
        myfile << it.first << ";" << it.second << ";" << endl;
    myfile.close();

    myfile.open("pararel_annealing/data/population2.csv", std::fstream::out);
    myfile << "threshold;energy" << endl;
    for (auto it: arg2->population){
        myfile << it.first << ";" << it.second << ";" << endl;
    }
    myfile.close();

    myfile.open("pararel_annealing/data/freq.csv", std::fstream::out);
    myfile << "gray scale;energy" << endl;
    for (int i = 0; i < 256; i++){
        myfile << i << ";" << objectiveFunc(i) << ';' << endl;
    }
    myfile.close();

    myfile.open("pararel_annealing/data/bests.csv", std::fstream::app);
